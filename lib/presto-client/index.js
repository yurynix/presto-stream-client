...
Client.prototype.pause = function(callback){
  if(!this._pause) {
    this._pause = true;
    ...
    return callback(new Error('query_not_running'));
  }
};

Client.prototype.isPaused = function() {
  return this._pause && this.hasOwnProperty('_onResume');
};

Client.prototype.resume = function() {
  if(this._pause && this.hasOwnProperty('_onResume')) {
    this._pause = false;
    this._onResume();
    delete this._onResume;
    return true;
  } else {
    return false;
  }
       ...
       * 2nd time
{
 ...
 "id": "20140120_032523_00000_32v8g"
      ...
      if(that._pause) {
        that._onResume = function(){
          fetch_next(next_uri);
        };
        if(that._onPaused && typeof that._onPaused === 'function') {
          that._onPaused(null);
          delete that._onPaused;
        }
        return;
      }
