 ...
 */
async function makeRequest(opts,contentBody,useSSL,jsonParser) {
    ...
    if (response.code === 503 ) { // https://github.com/prestodb/presto/wiki/HTTP-Protocol "If the response is an HTTP 503, sleep 50-100ms and try again"
        ...
        return makeRequest(opts,contentBody,useSSL,jsonParser);
    } else if(response.response_code < 300 && response.response_type === 'application/json' && response.data.length > 1) {
        try {
            ...
            throw new prestoError('request failed: unintelligible response.',response);
        }
    } else if (response.response_code < 300) { //some apis such as DELETE do not have a body
        return { response_code : response.response_code , data: {}};
    } else {
        throw new prestoError('request failed.',response);
    }
}
/**
...
class Client {
    ...
    constructor(args = {}){
        ...
        this[s_request] = async (opts)=>{
            ...
            return await makeRequest(requestOpts,contentBody,this[s_ssl],this.jsonParser);
        };
    ...
    async nodes(failed = false) {
        ...
        const { data } = await this[s_request]({ method: 'GET', path });
        return data;
    }
    /**
     ...
     */
    async cluster() {
        const { data } = await this[s_request]({ method: 'GET' , path: '/v1/cluster'});
        return data;
    }
    ...
    async status(query_id) {
        const { data } = await this[s_request]({ method: 'GET', path: '/v1/query/' + query_id });
        return data;
    }
    /**
     ...
     */
    async kill(query_id) {
        await this[s_request]({ method: 'DELETE', path: '/v1/query/' + query_id });
        return;
    }
    /**
    ...
    async execute(opts){
        ...
        const { response_code , data , session } = await this[s_request]({ method: 'POST', path: '/v1/statement', headers: header, body: opts.query });
...
class Statement extends Readable {
     ...
     */
    constructor(readableOptions,client,initialUri,queryId,fetchInfo = false,pollInterval = QUERY_STATE_CHECK_INTERVAL,session){
    ...
    async _run() {
        try {
            //const {hostname:host , port, pathname:path} = new URL(this[s_nextUri]);
            ...
            const { response_code , data , session , clear_session} = await this.client[s_request](requestOpts);
            if(this._statementCancelled()){
            ...
            */
            ...
            if(data.nextUri) {
                ...
                if(this.fetchInfo && data.infoUri) {
                    ...
                    const { data : info } = await this.client[s_request]({ path : data.infoUri});
